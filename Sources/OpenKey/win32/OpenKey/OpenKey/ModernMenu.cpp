...
void ModernMenu::EnableAcrylic(HWND hwnd) {
    // V4.4: Windows version-aware rendering
    // - Windows 11: Use DWM System Backdrop (Acrylic) - works great
    // - Windows 10: No transparency (GDI+ alpha + acrylic doesn't work reliably)
    
    bool isWin11 = OpenKeyHelper::isWindows11OrGreater();
    
    // 1. Dark mode title bar (Windows 10 1809+)
    BOOL dark = m_isDarkMode ? TRUE : FALSE;
    DwmSetWindowAttribute(hwnd, 20, &dark, sizeof(dark)); 
    
    if (isWin11) {
        // Windows 11: Use modern DWM features
        
        // Extend frame into client area for Backdrop to show through
        MARGINS margins = { -1 };
