...
void initMacroMap(const Byte* pData, const int& size) {
    ...
    for (int i = 0; i < macroCount; i++) {
        ...
        macroMap[key] = data;
        std::cout << "[Macro] Init: key[0]" << std::hex << key[0] << " for text=" << macroText << std::dec << std::endl;
    }
}
...
bool findMacro(std::vector<Uint32>& key, std::vector<Uint32>& macroContentCode) {
    ...
    if (macroMap.find(key) != macroMap.end()) {
        ...
        std::cout << "[Macro] Exact match found!" << std::endl;
        return true;
    }
    // Logging if exact match failed
    std::cout << "[Macro] Exact match failed. key[0]=" << std::hex << (key.size() > 0 ? key[0] : 0) << std::dec << std::endl;
    if (vAutoCapsMacro) {
        _macroFlag = false;
        ...
        std::cout << "[Macro] Testing AutoCaps with key[0]=" << std::hex << key[0] << std::dec << std::endl;
        if (key.size() > 0 && modifyCaseUnicode(key[0], false)) {
            std::cout << "[Macro] Lowercased key[0] to=" << std::hex << key[0] << std::dec << std::endl;
            if (macroMap.find(key) != macroMap.end()) {
                macroContentCode.clear();
                MacroData data = macroMap[key];
                macroContentCode = data.macroContentCode;
                std::cout << "[Macro] AutoCaps match found!" << std::endl;
...
void initMacrosFromList(const std::vector<std::pair<std::string, std::string>>& macros) {
    macroMap.clear();
...
std::vector<std::pair<std::string, std::string>> getAllMacrosAsList() {
